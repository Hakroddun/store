...
public class EndpointsListener {
    ...
    private Map<RequestMappingInfo, HandlerMethod> getHandlerMethods(ContextRefreshedEvent event) {
        ...
        // Sort the handler methods by endpoint path
        Map<RequestMappingInfo, HandlerMethod> sortedHandlerMethods = handlerMethods.entrySet().stream()
                .sorted(Comparator.comparing(entry -> formatMethodPath(entry.getKey())))
                .collect(Collectors.toMap(
                        Map.Entry::getKey, Map.Entry::getValue, (oldValue, newValue) -> oldValue, LinkedHashMap::new));

        return sortedHandlerMethods;
    }
    ...
    private String formatMethodPath(RequestMappingInfo key) {
        return key.getPathPatternsCondition().toString().replace("[", "").replace("]", "");
    }
}
